...
・ 2) pivotより右側のグループをpivotより大きい値に、左側グループをpivotより小さい値に交換していく。
...
・ 2) ソート済みとソート済みの右隣の値を比較し、右隣の値が大きなら右隣もソート済みとする。

・ 3) ソート済みとソート済みの右隣の値を比較し、ソート済みの値が大きならソート済みと右隣を入れ替えていく。

・ 4) 全ての値がソートされるまで1から繰り返す。
...
・ 配列の先頭"a"と右隣"b"の値を比較しa-bで負数ならaをbの前に、正数ならaをbの後ろへ並び替える。
